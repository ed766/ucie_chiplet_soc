...
| GCC C / RV32 architectural differential | `make -C chiplet_extension firmware-c-closure firmware-c-coverage` | `docs/reference/compiled_firmware_verification.md` | 35 named programs plus 50 deterministic seeded executions, `178 / 178` points, `94 / 94` event-correlated crosses, split code/data images, precise traps/interrupts, APB/DMA checking, independent instruction/memory/CSR state including `mscratch`, `14 / 14` true RTL mutations, `10 / 10` trace-checker self-tests, and native code coverage | Repository-local ISS plus architectural-effect-aware Spike `12 / 12`, ACT4/Sail `45 / 45`, formal `3 / 3`, and external mutation matrix `5 / 5`; not RISC-V certification |
| Privileged architecture lane | `make -C chiplet_extension privileged-arch-check` | `docs/reference/privileged_architecture_validation.md` | Eight GCC/ISS scenarios, 32 points, and 16 same-window crosses for `misa`, `mtval`, trap vectoring, interrupt cause, and `MRET` | Project machine-mode subset, not a complete privileged architecture or certification suite |
