...
written to [`../docs/project_metrics.md`](/home/esgha/ucie_chiplet_soc/docs/project_metrics.md).
...
- Low-power proxy runs meeting expectation: `20 / 20`
...
the named tests that hit each metric.
...
The checked-in power-suite reports show `20 / 20` low-power proxy runs meeting
...
bins, retention bins, and selected transition/activity bins visited, including
active-traffic power transitions, queued-DMA sleep/resume, and memory-retention
scenarios.
...
structure and RTL hierarchy/control references; it does not contribute
functional coverage.
