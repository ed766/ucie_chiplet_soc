...
def main() -> int:
    ...
    privileged = read("privileged_arch_summary.csv")
    privileged_coverage = read("privileged_arch_coverage_summary.csv")
    privileged_crosses = read("privileged_arch_cross_coverage_summary.csv")
    ...
    lines = [
        ...
        f"| Spike CPU differential | `{tally(spike)}` | PC/instruction, register write, memory address/mask, and store-data comparison across ALU, ABI, CSR, memory-width, dependency, control-flow, and optimizer cases | `chiplet_extension/reports/rv32_external_iss_summary.csv` |",
        ...
        f"| Privileged GCC/ISS scenarios | `{tally(privileged)}` | `misa`, `mtval`, direct/vectored `mtvec`, interrupt causes, and `MRET` state restoration | `chiplet_extension/reports/privileged_arch_summary.csv` |",
        f"| Privileged functional/cross evidence | `{sum(row.get('hit') == '1' for row in privileged_coverage)} / {len(privileged_coverage)}` points; `{sum(row.get('hit') == '1' for row in privileged_crosses)} / {len(privileged_crosses)}` crosses | Same-window cause/value/vector/source/return interactions | `chiplet_extension/reports/privileged_arch_coverage_summary.csv` |",
        "",
